...
training_df <- as.data.frame(as.matrix(training_dtm))
training_df$type <- sample$type

testing_dtm <- clean_corpus_dtm(test_raw_data$text)
testing_df <- as.data.frame(as.matrix(testing_dtm))
testing_df$type <- test_raw_data$type
...
```{r algorithms}
# Creates the Random Forest Classifier using the sample training data
summary(training_df)
