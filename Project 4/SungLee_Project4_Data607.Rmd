...
sample <- combined[sample(nrow(combined), 3000), ]
...
# Strip out html tags
sample_raw_spam_data$text <- strip_html(sample_raw_spam_data$text)
sample_raw_ham_data$text <- strip_html(sample_raw_ham_data$text)
...
sample_raw_spam_data$text <- str_replace_all(sample_raw_spam_data$text, "https?:\\/\\/(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\\.[a-zA-Z0-9()]{1,6}\\b([-a-zA-Z0-9()@:%_\\+.~#?&//=]*)", " ")
sample_raw_ham_data$text <- str_replace_all(sample_raw_ham_data$text, "https?:\\/\\/(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\\.[a-zA-Z0-9()]{1,6}\\b([-a-zA-Z0-9()@:%_\\+.~#?&//=]*)", " ")


# Remove Email addresses
sample_raw_spam_data$text <- str_replace_all(sample_raw_spam_data$text, "\\S*@\\S*\\s?", " ")
sample_raw_ham_data$text <- str_replace_all(sample_raw_ham_data$text, "\\S*@\\S*\\s?", " ")


# Remove \n, \t, and &nbsp;
sample_raw_spam_data$text <- str_replace_all(sample_raw_spam_data$text, "\n", " ")
sample_raw_spam_data$text <- str_replace_all(sample_raw_spam_data$text, "\t", " ")
sample_raw_spam_data$text <- str_replace_all(sample_raw_spam_data$text, "&nbsp;", " ")

sample_raw_ham_data$text <- str_replace_all(sample_raw_ham_data$text, "\n", " ")
sample_raw_ham_data$text <- str_replace_all(sample_raw_ham_data$text, "\t", " ")
sample_raw_ham_data$text <- str_replace_all(sample_raw_ham_data$text, "&nbsp;", " ")


# Convert the text data to the character class
sample_raw_spam_data$text <- as.character(sample_raw_spam_data$text)
sample_raw_ham_data$text <- as.character(sample_raw_ham_data$text)

# Store the variables as corpora
spam_corpus <- corpus(sample_raw_spam_data$text)
ham_corpus <- corpus(sample_raw_ham_data$text)
...
This page https://www.r-bloggers.com/text-message-classification/ has code for using a Naïve Bayes classifier for classifying messages in addition to the Quanteda page linked earlier.
